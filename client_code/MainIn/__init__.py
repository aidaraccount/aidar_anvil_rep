...
from anvil.js.window import (
    location,
    updateLoadingSpinnerMargin,
    clearModelNavigation,
)
...
class MainIn(MainInTemplate):
    def __init__(self, **properties):
        ...
        if user is None:
            ...
            if user is None:
                ...
                and (datetime.today().date() - user["expiration_date"]).days
                > 0
            ):
                ...
                # 2. Hide navigation sidebar
                try:
                    anvil.js.call_js("navbar_noModel_noSubs", False)
                except Exception as e:
                    print(
                        f"[NAVBAR] Error calling navbar_noModel_noSubs: {str(e)}"
                    )
            ...
            if user["user_id"] is None:
                ...
                # model_id
                self.model_id = save_var(
                    "model_id",
                    anvil.server.call("get_model_id", user["user_id"]),
                )
                print(f"MainIn model_id: {self.model_id}")

                # watchlist_id
                self.watchlist_id = save_var(
                    "watchlist_id",
                    anvil.server.call("get_watchlist_id", user["user_id"]),
                )
    ...
    def form_show(self, **event_args):
        ...
        try:
            if self._pending_model_states is not None:
                self.call_js(
                    "syncInitialModelStates", self._pending_model_states
                )
                self._pending_model_states = None
        except Exception as e:
    ...
    def watchlists_click(self, link_watchlist_id, wl_link, **event_args):
        click_link(
            wl_link,
            f"watchlist_details?watchlist_id={link_watchlist_id}&artist_id=None",
            event_args,
        )
        self.reset_nav_backgrounds()
    ...
    def refresh_models_components(self):
        ...
        if len(model_ids) > 0:
            ...
            def _eff_pinned(m):
                try:
                    return bool(m.get("is_pinned", False)) or bool(
                        m.get("active_notifications", False)
                    )
                except Exception:
                    return False
            ...
            for orig_index, model in ordered_models:
                ...
                model_link.set_event_handler(
                    "click",
                    self.create_model_click_handler(
                        model["model_id"], model_link, model_container
                    ),
                )
                ...
                try:
                    get_dom_node(model_container).setAttribute(
                        "data-model-id", str(model["model_id"])
                    )
                    # Preserve original creation-order index for later client-side stable reordering
                    get_dom_node(model_container).setAttribute(
                        "data-order-index", str(orig_index).zfill(6)
                    )
                    get_dom_node(model_link).setAttribute(
                        "data-model-id", str(model["model_id"])
                    )
                    get_dom_node(options_link).setAttribute(
                        "data-model-id", str(model["model_id"])
                    )
                except Exception as e:
                    print(
                        f"[NAV MODELS] Could not set data-model-id attributes: {e}"
                    )

                # 5. Add both links to the container
                model_container.add_component(
                    model_link, expand=True
                ...
                try:
                    ...
                    active_notifications = bool(
                        model.get("active_notifications", False)
                    )
                except Exception:
                    is_pinned = False
        ...
        if getattr(self, "_form_visible", False):
            try:
                ...
                print(
                    f"[NAV MODELS] syncInitialModelStates call failed (visible): {e}"
                )
        ...
        for container in self.nav_models.get_components():
            if isinstance(container, anvil.FlowPanel):
                for component in container.get_components():
                    ...
                    if (
                        isinstance(component, Link)
                        and component.icon == "fa:angle-right"
                    ):
    ...
    def create_model_click_handler(
        self, model_id, model_link, model_container
    ):
        def handler(**event_args):
            self.models_click(
                model_id, model_link, model_container, **event_args
            )

        return handler

    def models_click(
        self, link_model_id, model_link, model_container, **event_args
    ):
        ...
        click_link(
            model_link, f"agent_artists?artist_id={temp_artist_id}", event_args
        )
    ...
    def create_settings_click_handler(self, model_id):
        def settings_click_handler(**event_args):
            ...
            routing.set_url_hash(
                f"model_profile?model_id={model_id}&section=Main",
    ...
    def update_no_notifications(self, **event_args):
        NoNotifications = json.loads(
            anvil.server.call("get_no_notifications", user["user_id"])
        )
        self.link_watchlists.text = (
            "WATCHLISTS (" + str(NoNotifications[0]["cnt"]) + ")"
        )
    ...
    def reset_nav_backgrounds(self, **event_args):
        ...
        if location.hash[:5] == "#home" or location.hash == "":
            ...
            location.hash[:15] == "#model_profile?"
            or location.hash[:13] == "#model_setup?"
        ):
            query_string = location.hash.split("?")[1]
            ...
            if model_id != "None":
                ...
                for container in self.nav_models.get_components():
                    ...
                    ) and container.tag == int(model_id):
                        container.background = "theme:Brown"
                        break
    ...
    def link_discover_ai_click(self, temp_artist_id=None, **event_args):
        artist_id = anvil.server.call(
            "get_next_artist_id", load_var("model_id")
        )
        click_link(
            self.link_discover_ai, f"artists?artist_id={artist_id}", event_args
        )
        self.reset_nav_backgrounds()
        self.link_discover_ai.background = "theme:Brown"
    ...
    def link_discover_rel_click(self, **event_args):
        click_link(
            self.link_discover_rel, "rel_artists?artist_id=None", event_args
        )
        self.reset_nav_backgrounds()
        self.link_discover_rel.background = "theme:Brown"
    ...
    def create_model_click(self, **event_args):
        ...
        click_link(
            self.create_model,
            "agent_artists?artist_id=create_agent",
            event_args,
        )
        self.reset_nav_backgrounds()
    ...
    def SearchBar_pressed_enter(self, **event_args):
        search_data = json.loads(
            anvil.server.call(
                "search_artist", user["user_id"], self.SearchBar.text.strip()
            )
        )
        ...
        if not search_data:
            alert(
                title="Artist is not found or missing",
        ...
        anvil.server.call(
            "sent_push_over",
            "SearchBar",
            f'User {user["user_id"]}: using SearchBar',
        )

    def SearchBar_focus(self, **event_args):
        anvil.server.call("pre_warm", "general.artists_names_norm")
        anvil.server.call(
            "pre_warm", "general.artists_names_norm_name_norm_trgm_idx"
        )
    ...
    def shorten_number(self, num):
        ...
        def shorten_single_number(n):
            ...
            if n is None or not (
                ...
                or (isinstance(n, str) and n.isdigit())
            ):
                return "-"
            n = int(n)
    ...
    def feedback_click(self, **event_args):
        alert(content=C_FeedbackForm(), large=True, buttons=[])
    ...
    def MainIn_delete_reference(self, model_id, artist_id):
        ...
        if c is True:
            ...
            print(
                "[MainIn_delete_reference] delete_reference executed on backend"
            )
    ...
    def MainIn_get_genre_sub_scores(self, model_id):
        ...
        try:
            ...
            print(
                f"[MainIn_get_genre_sub_scores] Retrieved genre sub-scores for model_id: {model_id}"
            )
            return result
        except Exception as e:
            print(
                f"[MainIn_get_genre_sub_scores] Error retrieving genre sub-scores: {e}"
            )
            raise

    # Bridge method for dynamic artist change from JavaScript
    def dynamic_artist_change(self, new_artist_id):
        """Bridge method to call dynamic artist change on the current DiscoverAgent form"""
        try:
            print(
                f"[MainIn_dynamic_artist_change] Changing to artist: {new_artist_id}"
            )
            ...
            if content_panel and len(content_panel.get_components()) > 0:
                ...
                # Check if it's a DiscoverAgent form and has the dynamic_artist_change method
                if hasattr(current_form, "dynamic_artist_change") and callable(
                    getattr(current_form, "dynamic_artist_change")
                ):
                    current_form.dynamic_artist_change(new_artist_id)
                    print(
                        "[MainIn_dynamic_artist_change] Successfully called dynamic_artist_change"
                    )
                else:
                    print(
                        f"[MainIn_dynamic_artist_change] Current form is {type(current_form)} and does not have dynamic_artist_change method"
                    )
            else:
                print(
                    "[MainIn_dynamic_artist_change] No form found in content_panel"
                )

        except Exception as e:
            print(
                f"[MainIn_dynamic_artist_change] Error calling dynamic_artist_change: {e}"
            )
            raise
    ...
    def MainIn_delete_model(self, model_id):
        ...
        try:
            ...
            print(
                f"[MainIn_delete_model] Error deleting model {model_id}: {e}"
            )
            res = "error"
        ...
        if res == "success":
            ...
            try:
                ...
                if current_hash.startswith(
                    "#model_profile?"
                ) or current_hash.startswith("#model_setup?"):
                ...
                # b) If currently on a Discover page and the active model matches
                    ...
                    current_hash.startswith("#artists?")
                    or current_hash.startswith("#rel_artists?")
                    or current_hash.startswith("#agent_artists?")
                ):
                    try:
                        active_mid = load_var("model_id")
                        if active_mid is not None and int(active_mid) == int(
                            model_id
                        ):
                            should_go_home = True
                    except Exception:
                ...
                print(
                    f"[MainIn_delete_model] Error evaluating navigation condition: {e}"
                )
    ...
    def MainIn_update_agent_pin(self, agent_id: int, is_pinned: bool) -> str:
        ...
        print(
            f"[MainIn_update_agent_pin] Updating agent pin for agent_id: {agent_id}, is_pinned: {is_pinned}"
        )
        result = anvil.server.call("update_agent_pin", agent_id, is_pinned)
        print(f"[MainIn_update_agent_pin] Result: {result}")
        return result

    def MainIn_update_agent_notification(
        self, agent_id: int, active_notifications: bool
    ) -> str:
        """Update the notification activation state for an agent (model)."""
        print(
            f"[MainIn_update_agent_notification] Updating notifications for agent_id: {agent_id}, active: {active_notifications}"
        )
        result = anvil.server.call(
            "update_agent_notification", agent_id, active_notifications
        )
