from ._anvil_designer import ModelProfile_oldTemplate
from anvil import *
import anvil.server
import anvil.users
...
class ModelProfile_old(ModelProfile_oldTemplate):
  ...
  def edit_icon_click(self, **event_args):
    if self.model_name.visible is True: 
      ...
      res = anvil.server.call('update_model_stats', self.model_id_view, self.model_name_text.text, self.model_description_text.text, True)
      if res == 'success':
        Notification("",
          title="Model updated!",
          style="success").show()
